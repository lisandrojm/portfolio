...
export function FloatingLabelTextarea({ label, ...props }: FloatingLabelTextareaProps) {
  ...
  return (
    <div className="relative">
      ...
      <textarea {...props} className="px-4.5 block w-full border-0 bg-transparent pt-5 text-orange shadow-sm ring-1 ring-inset ring-white focus:ring-2 focus:ring-inset focus:ring-blue sm:text-sm sm:leading-6" onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} />
    </div>
  );
}
